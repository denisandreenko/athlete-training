# Alicja — Nutrition (Ketogenic)

Standalone keto plan. General keto principles, the per-100 g food table, targeted-carb fueling, and electrolyte guidance are in `shared/keto_nutrition_reference.md`.

Diet style: ketogenic, 3 meals/day. Stats: 164 cm, 52 kg, 24 yo, ~23% body fat. Macros below are computed from per-100 g values × portion. The values are a working draft — refine against real intake and progress.

| Metric | Daily total (approx) |
|--------|-------------|
| Protein | ~135.9 g |
| Carbs | ~25.8 g |
| Fat | ~97.1 g |
| Fiber | ~19.7 g |
| Calories | ~1662 kcal |

> ⚠️ This corrected total is ~1,662 kcal — below her ~1,800 kcal maintenance. The previous "Frozen veggies 250 g" row in Meal 1 mistakenly used egg macros (+~31 g protein, +~24 g fat, +268 kcal), which inflated the day. To hit a small muscle-building surplus (~1,900 kcal, ~135–150 g protein) increase a protein/fat source — e.g. more pork or eggs, or a little more oil/butter.

---

## Meal 1

| Item | Protein (g) | Carbs (g) | Fat (g) | Fiber (g) | kcal |
|------|-------------|-----------|---------|-----------|------|
| Eggs (3 large) 165g | 20.8 | 1.2 | 15.7 | 0 | 236 |
| Frozen veggies 250g | 0 | 7.3 | 1.8 | 5 | 90 |
| Cheese (2 slices) 32g | 8 | 0 | 8.3 | 0 | 107 |
| Olive oil 5g | 0 | 0 | 5 | 0 | 45 |
| Butter 20g | 0.2 | 0.2 | 16.4 | 0 | 149 |
| **Meal total** | **29** | **8.7** | **47.2** | **5** | **627** |

## Meal 2

| Item | Protein (g) | Carbs (g) | Fat (g) | Fiber (g) | kcal |
|------|-------------|-----------|---------|-----------|------|
| Ground pork (cooked) 200g | 56 | 0 | 18.4 | 0 | 426 |
| Frozen veggies 200g | 0 | 5.8 | 1.4 | 4 | 72 |
| Sardines 30g | 7.8 | 0 | 2.2 | 0 | 50 |
| Sauerkraut / kimchi 40g | 0.4 | 0.6 | 0 | 1.2 | 8 |
| **Meal total** | **64.2** | **6.4** | **22** | **5.2** | **556** |

## Meal 3

| Item | Protein (g) | Carbs (g) | Fat (g) | Fiber (g) | kcal |
|------|-------------|-----------|---------|-----------|------|
| Cottage cheese 9% 200g | 34 | 6.8 | 18 | 0 | 326 |
| L-reuteri yogurt 100g | 3.2 | 0.5 | 3.2 | 2 | 43 |
| Cocoa powder 12g | 2.5 | 1.9 | 1.3 | 3.8 | 37 |
| Chia seeds 10g | 1.7 | 0.8 | 3.1 | 3.4 | 44 |
| Pumpkin seeds 5g | 1.3 | 0.7 | 2.3 | 0.3 | 29 |
| **Meal total** | **42.7** | **10.7** | **27.9** | **9.5** | **479** |

---

## Pre-workout supplements

* **Caffeine:** 200 mg
* **Citrulline malate:** 6 g
* **Beta-alanine:** 6 g
* **Magnesium malate:** 200 mg
* **Beef collagen:** 8,700 mg + vitamin C

## Daily supplements

* **D3 + K2:** 8000 IU D3 + 200 µg K2
* **Omega 3:** ~1200 mg EPA/DHA
* **Creatine:** 10 g
* **Daily salt (low-sodium blend):** NaCl 70% + KCl 30%, iodized
* **Magnesium glycinate stack:** 320 mg magnesium + 30 mg zinc + 2 mg copper + 4.2 mg B6
* **Brazil nuts:** 2

Confirm with bloodwork before sustained high-dose vitamin D. Fueling: mostly easy running and novice gym work run fine fat-adapted — targeted carbs are rarely needed (see `shared/keto_nutrition_reference.md`).
